    ...
    def __init__(
        ...
        **kwargs,
        ...
        if max_steps_per_episode is not None:
            env = LimitedTimestep(env, max_steps_per_episode)
        if to_tensor:
...
class LimitedTimestep(Wrapper):
    """A wrapper, which limits steps per episode."""

    def __init__(self, env, max_steps_per_episode):
        super().__init__(env)
        self.max_steps_per_episode = max_steps_per_episode

    def step(self, action, **kwargs):
        observation, reward, done, info = self.env.step(action, **kwargs)
        if self.env._cache.steps_done >= self.max_steps_per_episode:
            done = True
